...
class MathDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tgt = self.tokenizer.encode(sample["output"], max_length=self.max_len - 1)  # -1 для SOS/EOS
        
        # Добавляем [SOS] и [EOS] к target
        tgt_input = [SPECIAL_TOKENS["SOS"]] + tgt
        tgt_output = tgt + [SPECIAL_TOKENS["EOS"]]
        
        # Паддинг до одинаковой длины
        if len(tgt_input) < self.max_len:
            tgt_input = tgt_input + [SPECIAL_TOKENS["PAD"]] * (self.max_len - len(tgt_input))
        if len(tgt_output) < self.max_len:
            tgt_output = tgt_output + [SPECIAL_TOKENS["PAD"]] * (self.max_len - len(tgt_output))
        
        return {
            "src": torch.LongTensor(src),
            "tgt_input": torch.LongTensor(tgt_input[:self.max_len]),
            "tgt_output": torch.LongTensor(tgt_output[:self.max_len])
        }
